...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('#messages .chat-markdown h3').innerText(),'Why the keys stay separate');
  assert.equal(await page.locator('#messages .chat-markdown li').count(),2);
  assert.equal(await page.locator('#messages .chat-markdown tbody tr').count(),2);
  assert.equal(await page.locator('#messages .chat-markdown script').count(),0);
  await page.locator('#messages [data-copy-code]').click();
  ...
  await page.reload();await page.locator('#messages .chat-markdown table').waitFor();
  ...
  for(const theme of ['light','dark'])for(const width of [1440,880,390,320]){
   ...
   await page.locator('#messages .chat-markdown h3').scrollIntoViewIfNeeded();
