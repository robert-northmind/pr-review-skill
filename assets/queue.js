...
function queueCard(pr){
 ...
 return `<article class="pr-card queue-card" data-queue-pr="${esc(pr.url)}">
  <div class="pr-main"><div>${prIdentity(pr)}<a class="pr-title" target="_blank" rel="noopener" href="${esc(safeUrl(pr.url))}">${esc(pr.title)}</a><div class="pr-byline">${authorBadge(pr)}</div><div class="pr-meta">${prAge(pr)}<span>${esc(stateLabel)}</span>${pr.is_draft?'<span class="chip">Draft</span>':''}${triageBadge(pr)}</div></div><div class="pr-actions">${primary}${codeWorkspaceLink(pr)}${artifactLink(notesArtifact(pr.artifacts),'Open AI notes',pr)}${actionDisclosure(pr,'•••',menu)}</div></div>
