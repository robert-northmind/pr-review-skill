...
function renderReporting(){
 ...
 if(signature===reportSignature){revealLatestReportPeriod();return;}reportSignature=signature;
 ...
 <section class="report-chart-section" aria-label="Activity chart"><div class="report-chart-heading"><h3>${reportInterval==='weekly'?'Weekly':'Daily'} activity</h3><span class="muted">${reportDate(start)} – ${reportDate(end)} · <span class="review-key">● Reviewed</span> <span class="merge-key">● Yours merged</span></span></div><p class="muted">Select a ${reportInterval==='weekly'?'week':'day'} to see its PRs. ${reportInterval==='weekly'?'This week is still in progress.':''}</p><div class="report-chart">${bars.map((bar,index)=>{
  ...
  return `<button class="report-bar" data-report-period="${bar.start}" aria-pressed="${selected}" aria-label="${reportInterval==='weekly'?'Week of ':''}${reportDate(bar.start)}: ${bar.review.length} reviewed, ${bar.merge.length} yours merged${bar.end>=today?', in progress':''}"><span class="report-bar-count"><span class="review-key">${bar.review.length}</span><span class="merge-key">${bar.merge.length}</span></span><svg viewBox="0 0 40 90" aria-hidden="true"><rect class="review-fill" x="5" y="${90-reviewHeight}" width="13" height="${reviewHeight}" rx="2"/><rect class="merge-fill" x="22" y="${90-mergeHeight}" width="13" height="${mergeHeight}" rx="2"/></svg><span class="report-bar-date">${reportInterval==='weekly'||index%7===0||selected?esc(reportDate(bar.start)):new Date(bar.start+'T12:00:00Z').getUTCDate()}</span></button>`;
 }).join('')}</div></section>
